...
pub(crate) fn process_shader<T: spv::specialisation::ShaderTY>(
    ...
    if let Some((ty, offset, name)) = builder.get_push_constant() {
        let stage = match T::TY {
            ...
            spv::rspirv::spirv::ExecutionModel::TessellationControl => gpu::ShaderStages::TESSELLATION_CONTROL,
            spv::rspirv::spirv::ExecutionModel::TessellationEvaluation => gpu::ShaderStages::TESSELLATION_EVAL,
...
fn process_push_constant_ty(ty: &spv::DataType, name: Option<&str>, push_constant_names: &mut HashMap<String, (u32, gpu::ShaderStages, TypeId)>, mut offset: u32, stage: gpu::ShaderStages) {
    ...
    let set_stages_or_panic = |id: TypeId, e: &mut (u32, gpu::ShaderStages, TypeId), name: &str, offset: u32, stage: gpu::ShaderStages| {
        if id != e.2 {
            panic!("ERROR: Multiple push constant variables of different type share the same name {}", name);
        } else if e.0 != offset {
    ...
    match ty {
        spv::DataType::Primitive(p) => if let Some(name) = name {
            let id = id(p);
            let e = push_constant_names.entry(name.to_string()).or_insert((0, gpu::ShaderStages::empty(), id));
            set_stages_or_panic(id, e, name, offset, stage);
        },
        spv::DataType::Array(p, n) => if let Some(name) = name {
            ...
            let id = id(p);
            for i in 0..*n {
                let e = push_constant_names.entry(format!("{}{}", name, i)).or_insert((0, gpu::ShaderStages::empty(), id));
                set_stages_or_panic(id, e, name, offset, stage);
            }
        },
        spv::DataType::Struct(ty_id, name, names, fields) => {
            let e = push_constant_names.entry(name.to_string()).or_insert((offset, gpu::ShaderStages::empty(), *ty_id));
