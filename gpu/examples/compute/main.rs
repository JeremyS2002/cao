...
fn main() {
    ...
    let descriptor_layout = device.create_descriptor_layout(&gpu::DescriptorLayoutDesc {
        name: None,
        entries: &[
            gpu::DescriptorLayoutEntry::StorageBuffer { stage: 
    ...
    let descriptor_set = device.create_descriptor_set(&gpu::DescriptorSetDesc {
        ...
        entries: &[
            gpu::DescriptorSetEntry::Buffer(buffer.slice_ref(..)),
        ]
    }).unwrap();
    ...
    buffer.slice_ref(..).read(bytemuck::cast_slice_mut(&mut data)).unwrap();
