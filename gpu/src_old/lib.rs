...
pub struct Instance {
    ...
    pub(crate) validation_errors: Arc<RwLock<Vec<String>>>,
}
...
impl Instance {
    ...
    unsafe fn raw(desc: &InstanceDesc<'_>) -> Result<(Self, bool), Error> {
        ...
        let mut instance = Self {
            ...
            validation_errors: Arc::default(),
        };

        if instance.validation_layers.len() != 0 {
            // no point creating debug utils if we don't have any validation layers
            let p_user_data = Arc::get_mut(&mut instance.validation_errors).unwrap() as *mut RwLock<Vec<String>> as *mut c_void;
            instance.debug_utils = Some(utils::DebugUtils::new(&instance, p_user_data)?)
        }
