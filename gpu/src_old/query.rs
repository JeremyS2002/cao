...
impl TimeQuery {
    ...
    pub fn check_folded_times(
        &self,
        first_query: u32,
        query_count: u32,
    ) -> Result<Option<Vec<Duration>>, crate::Error> {
        Ok(self
            .check_results(first_query, query_count)?
            .map(|s| self.fold_stamps_to_times(s)))
    }
    ...
    pub fn check_paired_times(
        ...
        Ok(self
            .check_results(first_query, query_count)?
            .map(|s| self.pair_stamps_to_times(s)))
    }
    ...
    pub fn get_folded_times(
        &self,
        first_query: u32,
        query_count: u32,
    ) -> Result<Vec<Duration>, crate::Error> {
