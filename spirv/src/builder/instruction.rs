        ...
        match self {
            ...
            Instruction::LoadIn { index, ty, store } => {
                ...
                let spv_var = if let Some(&spv_var) = var_map.get(store) {
                    ...
                    let pointer_ty =
                        builder.type_pointer(None, rspirv::spirv::StorageClass::Function, ty);
            ...
            Instruction::VectorShuffle {
                ...
                dst,
                components,
            } => process_vector_shuffle(var_map, builder, src, dst, *components),
            Instruction::NewArray { store, ty, data } => {
                let len = PrimitiveVal::UInt(data.len() as u32)
                    .set_constant(builder)
                    .0;
                let element_ty = ty.raw_ty(builder);
                let spv_ty = builder.type_array(element_ty, len);
                let pointer_ty =
                    builder.type_pointer(None, rspirv::spirv::StorageClass::Function, spv_ty);
                let var = builder.variable(
                    pointer_ty,
                    None,
                    rspirv::spirv::StorageClass::Function,
                    None,
...
fn process_vector_shuffle(
    ...
    let dst_obj = if dst.1.is_vector() {
        ...
        let pointer = builder
            .access_chain(pointer_ty, None, src_var, Some(components[0]))
            .unwrap();
...
fn get_res_var(builder: &mut rspirv::dr::Builder, res_ty: u32) -> u32 {
    let p_ty = builder.type_pointer(None, rspirv::spirv::StorageClass::Function, res_ty);
