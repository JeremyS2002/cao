...
#[derive(Clone, Copy, Debug, spv::AsStructType)]
#[allow(dead_code)]
#[repr(C)]
pub struct Uniform {
    model: glam::Mat4,
...
fn main() {
    ...
    let vertex_spv = {
        ...
        let in_col = b.in_vec3(1, "in_color");

        let vk_pos = b.vk_position();
        let out_col = b.out_vec3(0, "out_color");

        let u_data = b.uniform::<SpvUniform>(0, 0, Some("u_data"));
        ...
        b.entry(spv::Stage::Vertex, "main", || {
            ...
            vk_pos.store(projection * view * model * b.vec4(pos.x(), pos.y(), pos.z(), 1.0));

            out_col.store(in_col.load());
        });
    ...
    let fragment_spv = {
        ...
        let in_col = b.in_vec3(0, "in_col");

        let out_col = b.out_vec3(0, "out_col");
